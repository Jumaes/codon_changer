...
surface = [15, 19, 20, 38, 41, 44, 56, 59, 60, 63, 70, 79, 86, 91,106,109]
...
class Triplet:
    def __init__(self,trip, verbose = False):
        ...
        # print ('altcodons now:', self.altcodons) #debugging
        self.altcodons_remove = []
        for item in self.altcodons:
            #print ('Now looking at codon: ',item) #debugging
            if item in bad_codons: self.altcodons_remove.append(item)
        for item in self.altcodons_remove:
            #print ('Removing bad codon %s from altcodon list.' %(item)) #debugging
            self.altcodons.remove(item)
        ...
        self.expaltcodons_remove = []
        for item in self.expaltcodons:
            if item in bad_codons: self.expaltcodons_remove.append(item)
        for item in self.expaltcodons_remove: self.expaltcodons.remove(item)
